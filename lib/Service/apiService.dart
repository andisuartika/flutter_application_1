...
class APIService{
  ...
  static logut() async{
    getPref();
    try{ 
      final response = await http.get(Uri.parse(host+'api/logout'),
         headers: {
  ...
  static Future<List<Product>> getProductBySeller() async{
    getPref();
    try{  
      final response = await http.get(Uri.parse(host+'api/product/seller/'+_id.toString()),
         headers: {
          'Authorization':'Bearer '+_token,
        }); 
      if(response.statusCode == 200){
  ...
  static Future<List<Category>> getCategory() async{
    getPref();
    try{  
      final response = await http.get(Uri.parse(host+'api/category'),
         headers: {
          'Authorization':'Bearer '+_token,
        }); 
      if(response.statusCode == 200){
  ...
  static Future<ErrorMSG> updateProduct(product, filepath) async {
    ...
    try {
      // var stream = http.ByteStream(DelegatingStream.typed(filepath.openRead()));
      var request = http.MultipartRequest('POST', Uri.parse(host+'api/product/'+id.toString()));
      // var multipartFile = http.MultipartFile("image", stream, length, filename: path.basename(filepath.path));
      request.fields['nama']=product['nama'];
  ...
  static Future<ErrorMSG> deleteProduct(id) async {
    ...
    try {
      // var stream = http.ByteStream(DelegatingStream.typed(filepath.openRead()));
      var request = http.MultipartRequest('DELETE', Uri.parse(host+'api/product/'+id.toString()));
      request.headers.addAll(
        {
          'Authorization':'Bearer '+_token,
        }
